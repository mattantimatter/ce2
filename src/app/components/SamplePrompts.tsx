        ...
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-8 pointer-events-none max-w-4xl w-full px-4 transition-all duration-300" style={{ marginLeft: '0px' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-2xl pointer-events-auto">
                {samplePrompts.map((prompt, index) => (
                    <button
                        ...
                        className={`flex items-center gap-3 px-6 py-4 backdrop-blur-sm rounded-2xl text-left text-sm transition-all duration-200 cursor-pointer ${
                            isLight 
                                ? 'bg-gray-200/70 border border-gray-300 text-gray-900 hover:bg-gray-300 hover:border-gray-400'
            ...
            <div className="flex flex-col items-center gap-4 w-full max-w-2xl pointer-events-auto">
                <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium ${isLight ? 'text-gray-700' : 'text-white/70'}`}>Generate Artifacts</span>
                    <span className={`px-2 py-0.5 bg-blue-500/20 border border-blue-500/30 rounded-full text-xs font-medium ${isLight ? 'text-blue-600' : 'text-blue-400'}`}>NEW</span>
                </div>
